...
bool Grid::is_water_tile(int x, int y) {
  return items_[x][y] == Grid_Item::water || items_[x][y] == Grid_Item::water_2;
}

bool Grid::is_above_river_tile(int x, int y) {
  if (x == 0) {
...
void Grid::generate_ground_tiles() {
  for (int x = 0; x < items_.size(); x++) {
...
void Grid::generate_river_tiles() {
  for (int y = 0; y < items_[0].size(); y++) {
    if (y == 0) {
...
void Grid::generate_mountain_tiles() {
